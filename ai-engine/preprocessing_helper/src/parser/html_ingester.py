...
TABLE_ELEMENTS = {"caption", "col", "colgroup", "table",
...
class CustomHtmlTarget():
    ...
    class Results():
        ...
        def __init__(self):
            ...
            # For human comprehension of short_text
    ...
    def start(self, tag, attrs) -> None:
        ...
        for attr in attrs:
            ...
            important_word_set = preprocessing_pipeline(
                attrs[attr], remove_stopwords=True, html_meta=True)
            if important_word_set:
                self.results.meta_words_of_interest = \
                    self.results.meta_words_of_interest.union(
                        important_word_set)
        if len(attrs_list) > 0:
    ...
    def data(self, data) -> None:
        remove_extra_newline_elem(
            self.results.short_text, self.results.full_text)
        ...
        if is_not_blank(data):
            ...
            data = preprocessing_pipeline(
                data, html_meta=False, remove_stopwords=True)
